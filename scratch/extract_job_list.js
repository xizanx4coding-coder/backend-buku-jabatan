...
const jobs = new Set();
rows.forEach((row, i) => {
  // Let's skip header rows
  if (i < 3) return;
  // Nama Jabatan is in column index 2 (corresponds to __EMPTY_1, i.e., index 2 when read as raw array)
  // Let's inspect the columns
  const rawJab = row[2]; // Let's check what index contains the job title
  if (rawJab && typeof rawJab === 'string') {
    const clean = rawJab.replace(/\r?\n|\r/g, ' ').replace(/\s+/g, ' ').trim();
    if (clean && clean !== 'NAMA JABATAN' && clean.length > 3) {
      jobs.add(clean);
    }
  }
});
...
console.log(`Found ${sortedJobs.length} unique jobs.`);
