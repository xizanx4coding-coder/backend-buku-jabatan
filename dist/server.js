...
function redactForAnonymous(record) {
    const copy = { ...record };
...
function convertEselonToLocal(es) {
    if (!es)
        return null;
    ...
    if (VALID_ESELON_CODES.has(clean))
        return clean;
...
function matchOPD(unitKerjaInduk, unitKerjaSpesifik) {
...
function getJenjangFungsional(namaJabatan, golongan) {
    const cleanJabatan = (namaJabatan || '').toLowerCase();
    const cleanGol = (golongan || '').trim();
    if (cleanJabatan.includes('ahli pertama'))
        return 'Ahli Pertama';
    if (cleanJabatan.includes('ahli muda'))
        return 'Ahli Muda';
    if (cleanJabatan.includes('ahli madya'))
        return 'Ahli Madya';
    if (cleanJabatan.includes('ahli utama'))
        return 'Ahli Utama';
    if (cleanJabatan.includes('pemula'))
        return 'Pemula';
    if (cleanJabatan.includes('terampil'))
        return 'Terampil';
    if (cleanJabatan.includes('mahir'))
        return 'Mahir';
    if (cleanJabatan.includes('penyelia'))
        return 'Penyelia';
    // Fallback based on Golongan for Fungsional
    if (cleanGol === 'II/a' || cleanGol === 'II/b')
        return 'Pemula';
    if (cleanGol === 'II/c' || cleanGol === 'II/d')
        return 'Terampil';
...
function mapBegawiToPejabat(p) {
    ...
    const isExcluded = activeJabLower.includes('guru') ||
...
async function fetchAllPegawaiFromBegawi() {
    try {
...
async function performBlockingFetch() {
    const allData = [];
    let page = 1;
    const perPage = 50;
    let hasMore = true;
...
app.get('/api/pejabat', auth_1.optionalAuth, async (req, res) => {
    ...
    if (process.env.INTEGRATION_BEGAWI_ENABLED === 'true') {
        try {
            ...
            // 1. OPD Filter
            if (opd) {
                mappedList = mappedList.filter((p) => p.opd === opd || p.kd === opd);
            }
            // 2. Eselon & Category Filter
            if (category === 'fungsional' || eselon === 'JF') {
                mappedList = mappedList.filter((p) => p.kode_eselon === 'JF');
            }
            else if (category === 'structural') {
                mappedList = mappedList.filter((p) => p.kode_eselon && VALID_ESELON_CODES.has(p.kode_eselon));
            }
            else if (category === 'all') {
                mappedList = mappedList.filter((p) => p.kode_eselon === 'JF' || (p.kode_eselon && VALID_ESELON_CODES.has(p.kode_eselon)));
            }
            else {
                if (eselon) {
                    if (eselon === '-') {
                        mappedList = mappedList.filter((p) => !p.kode_eselon || p.kode_eselon === '-');
                    }
                    else {
                        mappedList = mappedList.filter((p) => p.kode_eselon === eselon);
                    }
                }
                else {
            ...
            if (status) {
                if (status === 'Lowong') {
                    mappedList = mappedList.filter((p) => p.nama_pejabat === 'LOWONG');
                }
                else {
                    mappedList = mappedList.filter((p) => p.nama_pejabat !== 'LOWONG' && p.ket_status === status);
                }
            }
            // 4. Gender Filter
            if (gender) {
                mappedList = mappedList.filter((p) => p.jk_gender === gender);
            }
            // 5. Search Filter (name, NIP, or jabatan)
            if (search) {
                const q = search.toLowerCase().trim();
                mappedList = mappedList.filter((p) => (p.nama_pejabat && p.nama_pejabat.toLowerCase().includes(q)) ||
    ...
    try {
        ...
        const countRow = await (0, database_1.dbGet)(countSql, params);
        ...
        sql += ' LIMIT ? OFFSET ?';
        params.push(l, (p - 1) * l);
        const rows = await (0, database_1.dbAll)(sql, params);
...
app.get('/api/stats', async (req, res) => {
    if (process.env.INTEGRATION_BEGAWI_ENABLED === 'true') {
        try {
            ...
            let pltJF = 0;
            let lowongJF = 0;
            const jfJenjangCounts = {};
            const jfGenderCounts = {};
            ...
            const totalEselon = mappedList.filter((p) => p.kode_eselon && VALID_ESELON_CODES.has(p.kode_eselon)).length;
            const totalJF = mappedList.filter((p) => p.kode_eselon === 'JF').length;
    ...
    try {
        const totalRow = await (0, database_1.dbGet)('SELECT COUNT(*) as count FROM pejabat');
        // Eselon
        const eselonRows = await (0, database_1.dbAll)('SELECT kode_eselon, COUNT(*) as count FROM pejabat GROUP BY kode_eselon');
        const eselonCounts = {};
        const jfJenjangCounts = {};
        ...
        // Gender
        const genderRows = await (0, database_1.dbAll)('SELECT jk_gender, kode_eselon, COUNT(*) as count FROM pejabat GROUP BY jk_gender, kode_eselon');
        const genderCounts = {};
        const jfGenderCounts = {};
        ...
        // Competency averages
        const compRow = await (0, database_1.dbGet)('SELECT AVG(kompetensi_teknis) as avg_t, AVG(kompetensi_manajerial) as avg_m, AVG(kompetensi_social_kultural) as avg_s FROM pejabat WHERE nama_pejabat != \'LOWONG\'');
        // Totals
        const totalJFRow = await (0, database_1.dbGet)("SELECT COUNT(*) as count FROM pejabat WHERE kode_eselon = 'JF'");
        ...
        // Lowong & PLT
        const structLowongRow = await (0, database_1.dbGet)("SELECT COUNT(*) as count FROM pejabat WHERE nama_pejabat = 'LOWONG' AND (kode_eselon != 'JF' OR kode_eselon IS NULL)");
        const jfLowongRow = await (0, database_1.dbGet)("SELECT COUNT(*) as count FROM pejabat WHERE nama_pejabat = 'LOWONG' AND kode_eselon = 'JF'");
        // Count PLT from jabatan_lowong
        const pltRow = await (0, database_1.dbGet)("SELECT COUNT(*) as count FROM jabatan_lowong WHERE plt_nama IS NOT NULL AND plt_nama != ''");
...
app.get('/api/opd', async (req, res) => {
    ...
    try {
        const rows = await (0, database_1.dbAll)('SELECT kd, opd, COUNT(*) as count FROM pejabat WHERE kd IS NOT NULL AND opd IS NOT NULL GROUP BY kd ORDER BY opd ASC');
...
app.get('/api/pejabat/:nip', auth_1.optionalAuth, async (req, res) => {
    ...
    try {
        const record = await (0, database_1.dbGet)('SELECT * FROM pejabat WHERE nip = ?', [nip]);
        if (!record) {
...
app.post('/api/pejabat', auth_1.authenticateToken, (0, auth_1.requireRole)(['editor', 'admin']), async (req, res) => {
    const r = req.body;
    ...
    `;
        ...
        if (r.nama_jabatan_structural)
            nama_jabatan = r.nama_jabatan_structural;
        else if (r.nama_jabatan_fungsional)
            nama_jabatan = r.nama_jabatan_fungsional;
        await (0, database_1.dbRun)(insertSql, [
        ...
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'CREATE_PEJABAT', `Added official: ${r.nama_pejabat} (NIP: ${r.nip || '-'})`);
...
app.put('/api/pejabat/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['editor', 'admin']), async (req, res) => {
    const { id } = req.params;
    const r = req.body;
    if (!r.opd || !r.nama_pejabat) {
        return res.status(400).json({ error: 'Nama Pejabat dan Unit Kerja (OPD) wajib diisi.' });
    ...
    try {
      ...
      WHERE id = ?
    `;
        await (0, database_1.dbRun)(updateSql, [
            r.no, r.kd, r.opd, r.nama_jabatan_structural, r.nama_jabatan_fungsional, nama_jabatan, r.nama_pejabat,
            r.ket_status, r.nip, r.pangkat_gol_tmt, r.pendidikan, r.tmt_jabatan, r.mkj_terakhir, r.kode_eselon,
            r.tmt_eselon, r.mkj_eselon, r.ket, r.agama, r.jk_gender, r.nilai_kinerja, r.kompetensi_teknis,
            r.kompetensi_manajerial, r.kompetensi_social_kultural, r.kategori, r.tahun_kinerja,
        ...
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'UPDATE_PEJABAT', `Updated details for official: ${r.nama_pejabat} (ID: ${id})`);
...
app.post('/api/pejabat/:id/toggle-plt-status', auth_1.authenticateToken, (0, auth_1.requireRole)(['editor', 'admin']), async (req, res) => {
    const { id } = req.params;
    try {
        const record = await (0, database_1.dbGet)('SELECT status_plt_plh, nama_pejabat FROM pejabat WHERE id = ?', [id]);
        if (!record)
            return res.status(404).json({ error: 'Pejabat tidak ditemukan.' });
        const newStatus = record.status_plt_plh ? 0 : 1;
        await (0, database_1.dbRun)('UPDATE pejabat SET status_plt_plh = ? WHERE id = ?', [newStatus, id]);
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'TOGGLE_PLT_STATUS', `Toggled PLT/PLH status for ${record.nama_pejabat} to ${newStatus}`);
...
app.delete('/api/pejabat/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    const { id } = req.params;
    try {
        const target = await (0, database_1.dbGet)('SELECT nama_pejabat FROM pejabat WHERE id = ?', [id]);
        const name = target ? target.nama_pejabat : 'Unknown';
        await (0, database_1.dbRun)('DELETE FROM pejabat WHERE id = ?', [id]);
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'DELETE_PEJABAT', `Deleted official: ${name} (ID: ${id})`);
...
app.get('/api/users', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)('SELECT id, username, name, role FROM users');
        res.json(rows);
    }
    catch (err) {
...
app.post('/api/users', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    ...
    try {
        ...
        await (0, database_1.dbRun)('INSERT INTO users (username, password, name, role) VALUES (?, ?, ?, ?)', [username, passHash, name, role]);
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'CREATE_USER', `Created user account: ${username} (${role})`);
...
app.put('/api/users/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    const { id } = req.params;
    const { name, role, password } = req.body;
    try {
        if (password && password.trim()) {
            const passHash = bcrypt.hashSync(password, 10);
            await (0, database_1.dbRun)('UPDATE users SET name = ?, role = ?, password = ? WHERE id = ?', [name, role, passHash, id]);
        }
        else {
            await (0, database_1.dbRun)('UPDATE users SET name = ?, role = ? WHERE id = ?', [name, role, id]);
        }
        const target = await (0, database_1.dbGet)('SELECT username FROM users WHERE id = ?', [id]);
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'UPDATE_USER', `Updated user account settings: ${target?.username}`);
...
app.delete('/api/users/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        const target = await (0, database_1.dbGet)('SELECT username FROM users WHERE id = ?', [id]);
        await (0, database_1.dbRun)('DELETE FROM users WHERE id = ?', [id]);
        await (0, database_1.createAuditLog)(req.user?.username || 'system', 'DELETE_USER', `Deleted user account: ${target?.username}`);
...
app.get('/api/audit-logs', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)('SELECT * FROM audit_logs ORDER BY timestamp DESC LIMIT 500');
        res.json(rows);
    }
    catch (err) {
...
app.put('/api/settings', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    ...
    try {
        const user = req.user;
        await (0, database_1.createAuditLog)(user.username, 'UPDATE_SETTINGS', `Memperbarui pengaturan integrasi. Status: ${process.env.INTEGRATION_BEGAWI_ENABLED}`);
    }
    catch (logErr) {
...
app.get('/api/export/excel', auth_1.authenticateToken, async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)('SELECT * FROM pejabat ORDER BY no ASC');
...
app.get('/api/jabatan-lowong', async (req, res) => {
...
app.get('/api/jabatan-lowong/stats', async (req, res) => {
    try {
        const byLevel = await (0, database_1.dbAll)(`SELECT level,
              COUNT(*) AS total,
              COUNT(CASE WHEN plt_nama IS NOT NULL AND plt_nama != '' THEN 1 END) AS dengan_plt
       FROM jabatan_lowong
       GROUP BY level`);
        const byOpd = await (0, database_1.dbAll)('SELECT opd, level, COUNT(*) AS total FROM jabatan_lowong GROUP BY opd, level ORDER BY opd');
...
app.get('/api/jabatan-lowong/eselon2', async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)("SELECT * FROM jabatan_lowong WHERE level = 'eselon2' ORDER BY no ASC");
        res.json({ total: rows.length, data: rows });
    }
    catch (err) {
...
app.get('/api/jabatan-lowong/eselon3', async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)("SELECT * FROM jabatan_lowong WHERE level = 'eselon3' ORDER BY no ASC");
        res.json({ total: rows.length, data: rows });
    }
    catch (err) {
...
app.get('/api/jabatan-lowong/eselon4', async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)("SELECT * FROM jabatan_lowong WHERE level = 'eselon4' ORDER BY opd, no ASC");
        res.json({ total: rows.length, data: rows });
    }
    catch (err) {
...
app.get('/api/pensiun', async (req, res) => {
    try {
        const rows = await (0, database_1.dbAll)('SELECT * FROM pejabat_pensiun ORDER BY no ASC');
        res.json({ total: rows.length, data: rows });
    }
    catch (err) {
